...
const HomeScreen = ({ route }) => {
  ...
  useEffect(() => {
    const fetchUserProjects = async () => {
      try {
        ...
        if (userEmail) {
          const response = await axios.get(`http://10.55.6.135:8000/api/get-user-projects/${userEmail}/`);
  ...
  const handleLogout = async () => {
    try {
      const response = await axios.get(`http://10.55.6.135:8000/api/logout/${email}/`);
      console.log(response.data);
