...
useEffect(() => {
  const fetchData = async () => {
    try {
      // Fetch step details
      const stepResponse = await axios.get(`http://10.55.5.124:8000/api/get_step_details/${stepId}/`);
  ...
  const handleEditStep = async () => {
    try {
      ...
      const response = await axios.post(`http://10.55.5.124:8000/api/edit-step/${stepId}/`, dataToSend, {
        headers: {
